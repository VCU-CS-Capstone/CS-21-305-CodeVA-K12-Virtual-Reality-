...
You will notice that some instruments that came with the Unity Asset Store Instrument pack are not present in our game. You are free to pick whichever instruments you want to use for your game. You will also notice that some of the instruments we used in our game are either not in the default pack, like the snare drum, or are missing parts in their default states, such as the DJ mixer not having headphones, the drums not having drum sticks, or the cymbals being only a single cymbal. You can use the instruments in their default states if you wish, or if you want a little extra challenge, you can make the instruments look like ours. We will leave this to be a creative task for you. <i>(HINT: Mutiple instruments/other empty game objects can be children of the parent game object to create a new prefab. Make sure to layer instruments and game objects correctly so that they work with your scripts.)</i> </br>

9. Once all of your other instruments are saved a prefabs, we can create outlines to be used for the other scenes in the music project. Duplicate the Music Main Menu scene twice and name the results as follows: Finite Game and Gallery. Change the Exit buttons in the new scenes to point back to the Music Main Menu scene rather than the general Main Menu scene. <i><b>NOTE:</b> There will eventually be an Infinite Game scene present as well; however, it is easier to set up the spawner once in the Finite Game scene and then Duplicate it to create the Infinite Game scene than it is to do that work twice.)</i>
