...
Next, we want to create the Buttons for the player to be able to select the instruments when they spawn. Therefore, we will create a Canvas for the scene and call it "StageButtonCanvas." To ensure the Canvas has all proper components attached, you can duplicate another previously existing canvas and change UI elements accordingly. Under the canvas create three "Button - TextMeshPro" buttons and name them "Left", "Middle" and "Right" respectively. Your Hierarchy should look like this:
